...
#Region "Load"
    ...
    Public Sub LoadDepartment()
        Using conn As SQLiteConnection = DatabaseHandler.GetConnection()
            Try
                    conn.Open()
                Dim cmd As New SQLiteCommand("SELECT Departments FROM [Departments]", conn)
                Dim reader As SQLiteDataReader = cmd.ExecuteReader()
                Dim items As New List(Of String)()
    ...
    Public Sub LoadGageType()
        Using conn As SQLiteConnection = DatabaseHandler.GetConnection()
            Try
                    conn.Open()
                Dim cmd As New SQLiteCommand("SELECT GageType FROM [GageType]", conn)
                Dim reader As SQLiteDataReader = cmd.ExecuteReader()
                Dim items As New List(Of String)()
    ...
    Public Sub LoadCustomers()
        Using conn As SQLiteConnection = DatabaseHandler.GetConnection()
            Try
                    conn.Open()
                Dim cmd As New SQLiteCommand("SELECT CustomerName FROM Customers", conn)
                Dim reader As SQLiteDataReader = cmd.ExecuteReader()
                Dim items As New List(Of String)()
...
#Region "GTMenu Buttons"
    ...
    Private Sub BtnUpdateConfirmed()
        Using conn As SQLiteConnection = DatabaseHandler.GetConnection()
            ...
            Dim updateCmd As New SQLiteCommand("UPDATE [CalibrationTracker] SET PartNumber = ?, PartRev = ?, Status = ?, Description = ?, Department = ?, [Gage Type] = ?, Customer = ?, [Calibrated By] = ?, [Interval (Months)] = ?, [Inspected Date] = ?, [Due Date] = ?, Comments = ?, aN1 = ?, aN2 = ?, aN3 = ?, aN4 = ?, aN5 = ?, aA1 = ?, aA2 = ?, aA3 = ?, aA4 = ?, aA5 = ?, [Serial Number] = ?, Owner = ?, [Nist Number] = ?, [Last User] = ?, [Last Edited] = ? WHERE GageID = ?", conn)
            ...
            updateCmd.Parameters.AddWithValue("@LastUser", lastUser)
            updateCmd.Parameters.AddWithValue("@LastEdited", lastEdited)
...
#Region "Misc"
    ...
    Private Sub DeleteConfirmed()
        ...
        If result = DialogResult.Yes Then
            Try
                Using conn As SQLiteConnection = DatabaseHandler.GetConnection()
                    conn.Open()
                    Dim deleteCmd As New SQLiteCommand("DELETE FROM [CalibrationTracker] WHERE GageID = ?", conn)
    ...
    Private Sub SearchAuditLog()
        Try
            Using conn As SQLiteConnection = DatabaseHandler.GetConnection()
                conn.Open()
                Dim searchCmd As New SQLiteCommand("SELECT [Date Added], [Last Edited], [Last User] FROM [CalibrationTracker] WHERE GageID = ?", conn)
